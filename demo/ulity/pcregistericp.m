...
if isempty(p)
    ...
    p.addParameter('InitialTransform', defaults.InitialTransform, ...
        @(x)validateattributes(x,{'rigid3d', 'affine3d'}, {'scalar'}));
...
metric          = validatestring(parser.Results.Metric, {'PointToPoint', 'PointToPlane'}, funcName, 'Metric');
...
if isa(initialTransform, 'affine3d')
    initialTransform = rigid3d(initialTransform.T);
end
